...
module extruder_carriage(bearing, show_bearing=false)
{
	...
	p_t = 3.5;		// Main plate thickness

	ex_hole_t = p_t + 0.2;

	p_w = (2 * bb_d) + ex_hole_w + ex_hole_space;	// ... width
	p_l = 80;										// ... length

	v = [
		p_l, p_w, p_t, 
		bearing, bb_w, bb_d, bb_h, bb_g, bb_len, 
		ex_hole_l, ex_hole_w, ex_hole_t];
