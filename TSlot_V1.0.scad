			...
			for(x=[0:xcount-1]) {
				for(y=[0:ycount-1]){ 
					translate([dimension*x-dimension,dimension*y-dimension,0])
						tslot(size=dimension,gap=2.63*(dimension/10),thickness=.75*(dimension/10),length=length);
				}
			}
		}
}
			...
			translate([size,size])
				difference(){
					union(){
						for(d=[0:3])
							rotate([0,0,d*90])
								tslot_side(size, thickness, gap, start);
...
module tslot2020(length){
	tslot_build(20,1,1,length);
}
module tslot2040(length){
	tslot_build(20,1,2,length);
}
module tslot2060(length){
	tslot_build(20,1,3,length);
}
module tslot4040(,length){
	tslot_build(20,2,2,length);
}
module tslot4080(length){
	tslot_build(20,2,4,length);
}
